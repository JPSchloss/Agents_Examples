...
Rules:
  ...
  - Keep any message of your own to one short sentence."""
    ...
    model=config.MODEL,
    # Use handoff() with explicit, valid tool names so agent display names can stay
    # human-readable ("Data Engineer") without emitting invalid-tool-name warnings.
    handoffs=[
...
# Let each specialist hand control back to Triage (cyclic references, so set after).
