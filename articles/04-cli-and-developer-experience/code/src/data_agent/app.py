...
async def cmd_chat(args: argparse.Namespace) -> None:
    ...
    async with AsyncExitStack() as stack:
        ...
        _print_banner(mcp_status, with_knowledge)
...
# REPL
# --------------------------------------------------------------------------------------
def _print_banner(mcp_status: str, with_knowledge: bool) -> None:
    ...
    console.print(
        f"[dim]model:[/] {config.MODEL}   [dim]rag:[/] {rag_status}   [dim]mcp:[/] {mcp_status}\n"
    )


async def _repl(
    triage, context: PipelineContext, session: SQLiteSession, max_turns: int
) -> None:
    while True:
        try:
        ...
        try:
            with console.status("[dim]thinking…[/]", spinner="dots"):
                result = await Runner.run(
                    triage, user, context=context, session=session, max_turns=max_turns
                )
            console.print(
                Panel(
