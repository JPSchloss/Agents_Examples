...
from ..safety import dangerous_code_guardrail
...
@function_tool(tool_input_guardrails=[dangerous_code_guardrail])
def write_file(ctx: RunContextWrapper[PipelineContext], path: str, content: str) -> str:
    ...
    # Human-in-the-loop: ask before writing (unless --auto-approve / non-interactive).
    detail = f"path: {path}\n--- content preview ---\n{content[:800]}"
    if not c.approve("write_file", detail):
...
def run_python_file(ctx: RunContextWrapper[PipelineContext], path: str) -> str:
    ...
    # Human-in-the-loop: ask before executing code (unless --auto-approve / non-interactive).
    if not c.approve("run_python_file", f"execute workspace/{target.relative_to(c.workspace_dir)}"):
        return "DECLINED by user: script not run. Adjust the plan or ask the user."
